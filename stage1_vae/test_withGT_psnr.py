...
def compute_psnr(image1, image2):
    epsilon = 1e-8
    mse = torch.mean((image1 - image2)**2 + epsilon)
...
if __name__ == "__main__":
    ...
    if opt.with_GT:
        gts = []
        for root, dirs, files in os.walk(os.path.join(opt.indir,'LR')):
            for file in files:
                image_path = os.path.join(root, file)
                images.append(image_path)
        for root, dirs, files in os.walk(os.path.join(opt.indir,'GT')):
            for file in files:
    ...
    with torch.no_grad():
        for image in tqdm(images):
            ...
            if opt.with_GT:
                gt_path = filepath.replace('LR', 'GT')
                gt = os.path.join(gt_path,filename)
                gt_batch, gt_h, gt_w, gt_size = make_batch(gt, device=device)
            ...
            if opt.with_GT:
                psnr = compute_psnr(gt_batch, predicted_image)
