...
class Decoder_Mix(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.give_pre_end = give_pre_end 
        self.tanh_out = tanh_out
        self.fusion_w = fusion_w #1

        # compute in_ch_mult, block_in and curr_res at lowest res
        in_ch_mult = (1,)+tuple(ch_mult) #(1,1,2,4,4)
        block_in = ch*ch_mult[self.num_resolutions-1] # 512
        curr_res = resolution // 2**(self.num_resolutions-1) #
        self.z_shape = (1,z_channels,curr_res,curr_res) #64
        print("Working with z of shape {} = {} dimensions.".format(
            self.z_shape, np.prod(self.z_shape))) #np.prod(self.z_shape) = 64*64*3
        ...
        for i_level in reversed(range(self.num_resolutions)): #3,2,1,0
            block = nn.ModuleList()
            attn = nn.ModuleList()
            block_out = ch*ch_mult[i_level] #[128*4,128*4,128*2,128]

            if i_level != self.num_resolutions-1: # not the last layer
                if i_level != 0:
                    fuse_layer = Fuse_sft_block_RRDB(in_ch=block_out, out_ch=block_out, num_block=num_fuse_block) # 2 fuse_block
                    setattr(self, 'fusion_layer_{}'.format(i_level), fuse_layer)
                    # fusion_layer_2, 2 fuse_block
                    # fusion_layer_1, 2 fuse_block
    ...
    def forward(self, z, enc_fea):
        ...
        # upsampling
        for i_level in reversed(range(self.num_resolutions)):# 3,2,1,0
            for i_block in range(self.num_res_blocks+1): #i_block=0,1,2
                h = self.up[i_level].block[i_block](h, temb) # h=self.up[3].block[0]
                if len(self.up[i_level].attn) > 0:
                    h = self.up[i_level].attn[i_block](h)
...
class Fuse_sft_block_RRDB(nn.Module):
    ...
    def forward(self, enc_feat, dec_feat, w=1):
        print("enc_feat, dec_feat",enc_feat.shape, dec_feat.shape)
