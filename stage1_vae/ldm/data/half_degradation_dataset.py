...
class RdmgridDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(RdmgridDataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        self.repeat_times = self.opt['repeat_times']
        ...
        self.lq = degra_shffle_imgStack

        # repeat gt for eight times
        lt=[]
        for i in range(0,self.gt.size(0)):
            gt_aug = self.gt[i].unsqueeze_(0).repeat(self.repeat_times,1,1,1)
            gt_aug = gt_aug.unsqueeze_(0)
            for t in gt_aug:
                lt.append(t)
        self.gt = torch.cat(lt,dim=0)
        
        # self.gt: rgb, nchw, 0,1
        # hq: rgb, hwc, -1,1
        hq = (self.gt.permute(0, 2, 3, 1) * 2 - 1).contiguous().numpy().astype(np.float32)
        #?? question there! can I do normalize in this way? include the batch 

        # self.lq: rgb, nchw, 0,1
        # lq: rgb, hwc, 0,1
        lq = (self.lq.permute(0, 2, 3, 1) * 2 - 1).contiguous().numpy().astype(np.float32)
        return dict(image=lq, txt="", target=hq)
