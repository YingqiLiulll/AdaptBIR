...
def process(model, control_imgs, height, width, steps, guess_mode, strength, scale,
            color_fix_type, disable_preprocess_model):
    ...
    with torch.no_grad():
        ...
        if disable_preprocess_model:
            ...
            z = model.apply_denoise_encoder(control).mode()
            c_latent = model.first_stage_model.post_quant_conv(z)
        ...
        un_cond = {"c_concat": None if guess_mode else [control], "c_crossattn": [model.get_learned_conditioning([""] * n_samples)]}
        ...
        init_image = (control * 2 - 1).clamp(-1, 1) # RGB, NCHW, -1,1
...
def get_denoiseAE_preds(model, lq_imgs):
    ...
    c_latent = model.first_stage_model.post_quant_conv(z)
    control_output = model.first_stage_model.decoder(c_latent)
    control_output = (einops.rearrange(control_output, 'b c h w -> b h w c') * 255.0).cpu().numpy().clip(0, 255).astype(np.uint8)
    ...
    for file in files:
        ...
        for i in range(args.repeat_times):
            ...
            ddpm_preds = process(
                ...
                guess_mode=False, strength=1, scale=1.0,
                color_fix_type=args.color_fix_type,
                disable_preprocess_model=args.disable_preprocess_model
            )
            control_img = (control_imgs[0] * 255).clip(0, 255).astype(np.uint8)
            ddpm_pred = ddpm_preds[0]
            if not args.disable_preprocess_model:
                denoiseAE_pred = get_denoiseAE_preds(model, control_imgs)[0]
            else:
