...
class ControlledSFTUnetModel(UNetModel):
    ...
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, control_feature=None, **kwargs):
        ...
        for i, module in enumerate(self.output_blocks):
            ...
            if h.shape[1] == 1280:
                h[:,:1280] = h[:,:1280] * self.b1
                control_feature[-1] = Fourier_filter(control_feature[-1], threshold=1, scale=self.s1)
            if h.shape[1] == 640:
                h[:,:640] = h[:,:640] * self.b2
                control_feature[-1] = Fourier_filter(control_feature[-1], threshold=1, scale=self.s2)
            # ---------------------------------------------------------
            if h.shape[1] == 320:
                h[:,:320] = h[:,:320] * self.b3
                # control_feature[-1]=torch.zeros_like(control_feature[-1])
                control_feature[-1] = Fourier_filter(control_feature[-1], threshold=1, scale=self.s3)
