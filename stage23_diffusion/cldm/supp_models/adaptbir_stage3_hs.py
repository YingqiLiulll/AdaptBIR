    ...
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, control_feature=None, csft_w=None,**kwargs):
        ...
        for i, module in enumerate(self.output_blocks):
            ...
            # if not only middle control,then the control will also work on the decoder
                ...
                hs[-1]= self.cft_blocks[idx](control_feature.pop(),hs[-1],csft_w)
...
class ControlLDM(LatentDiffusion):
    ...
    # def calculate_musiq(self, data, device=None, **kargs):
    #     img = data
    #     if device is None:
    #         device = 'cuda' if torch.cuda.is_available() else 'cpu'
    #     musiq_model = pyiqa.create_metric('musiq', device=device, as_loss=False)
    #     print("musiq_img.shape:",img.shape)
    #     score = musiq_model(img[None,...])
    ...
    def get_input(self, batch, k, sample_posterior=False, bs=None, *args, **kwargs):
        ...
        # score = self.musiq_model(control)
        # musiq_score = float(score.detach().cpu().numpy())
        ...
        for i in range(0,control.shape[0]):
            # musiq_score = self.calculate_musiq(control[i])
            img = control[i]
        ...
        if sample_posterior:
            ...
            z = posterior.mode()
        c_latent1 = self.denoise_post_quant_conv(z)
        ...
        # print("get input use time:", start.elapsed_time(end))
