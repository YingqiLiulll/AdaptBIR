    ...
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, **kwargs):
        ...
        for i, module in enumerate(self.output_blocks):
            ...
            # if not only middle control,then the control will also work on the decoder
                ...
                if hs[-1].shape[1] == 1280:
                    # hs[-1] = Fourier_filter(hs[-1], threshold=1, scale=self.b1)
                    hs[-1][:,:1280] = hs[-1][:,:1280] * self.b1
                    # control[-1][:,:1280] = control[-1][:,:1280] * self.s1
                    control[-1] = Fourier_filter(control[-1], threshold=1, scale=self.s1)
                if hs[-1].shape[1] == 640:
                    # hs[-1] = Fourier_filter(hs[-1], threshold=1, scale=self.b2)
                    hs[-1][:,:640] = hs[-1][:,:640] * self.b2
                    # control[-1][:,:640] = control[-1][:,:640] * self.s2
                    control[-1] = Fourier_filter(control[-1], threshold=1, scale=self.s2)
                if hs[-1].shape[1] == 320:
                    # hs[-1] = Fourier_filter(hs[-1], threshold=1, scale=self.b3)
                    hs[-1][:,:320] = hs[-1][:,:320] * self.b3
                    # control[-1][:,:320] = control[-1][:,:320] * self.s3
                    control[-1] = Fourier_filter(control[-1], threshold=1, scale=self.s3)
