...
from ldm.modules.diffusionmodules.openaimodel import TimestepEmbedSequential, ResBlock, Downsample, AttentionBlock, UNetModel, UNetModelDualcondV2
...
class ControlNet(nn.Module):
    def __init__(
        ...
        self.zero_convs = nn.ModuleList([self.make_zero_conv_spade(model_channels,out_channels)])
        ...
        for level, mult in enumerate(channel_mult):
            ...
            for nr in range(self.num_res_blocks[level]):
                ...
                self.zero_convs.append(self.make_zero_conv_spade(ch,out_channels))
        ...
        self.middle_block_out = self.make_zero_conv_spade(ch,out_channels)
        self._feature_size += ch

    def make_zero_conv(self, channels):
        return TimestepEmbedSequential(zero_module(conv_nd(self.dims, channels, channels, 1, padding=0)))

    def make_zero_conv_spade(self, channels, out_channels):
...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(self, control_stage_config, control_key, preprocess_config, unfrozen_diff=False, *args, **kwargs):
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        if cond['c_latent'] is None:
            eps = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, struct_cond=None)
        else:
            ...
            eps = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, struct_cond=control)
    ...
    def configure_optimizers(self):
        ...
        for name,param in self.model.named_parameters():
        #     # if param.is_leaf == 'false':
            # print("self.model:",name, param.requires_grad, param.is_leaf, param.device)
            if 'spade' in name:
                # params = params+ list(param)
                params.append(param)
                # print("we add spade param!!!",name)
