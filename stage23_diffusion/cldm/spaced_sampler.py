...
class SpacedSampler:
    ...
    def sample(self,
        ...
        self.make_schedule(num_steps=S)
        # sampling
        C, H, W = shape
        size = (batch_size, C, H, W)
        print(f'Data shape for Spaced sampling is {size}')

        samples, intermediates = self.sapced_sampling(
            conditioning, size, x_T=x_T, log_every_t=log_every_t,
    ...
    @torch.no_grad()
    def sapced_sampling(self, cond, shape, x_T=None, log_every_t=100, 
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_spaced(img, cond, ts, index=index, unconditional_guidance_scale=unconditional_guidance_scale,
                                        unconditional_conditioning=unconditional_conditioning,
                                        cond_fn=cond_fn)
            img, pred_x0 = outs
    ...
    @torch.no_grad()
    def p_sample_spaced(self, x: torch.Tensor, c, t, index, unconditional_guidance_scale=1.,
                        unconditional_conditioning=None, cond_fn=None):
        index = torch.full_like(t, fill_value=index)
        
        c = c.copy()
        if unconditional_conditioning is not None:
            uc = unconditional_conditioning.copy()
        else:
            uc = None
        ...
        if cond_fn is not None:
            model_mean, pred_x0 = self.apply_cond_fn(
                x, c, t, index, cond_fn,
                unconditional_guidance_scale, uc
            )
        else:
            e_t = self.predict_noise(
                x, t, c, unconditional_guidance_scale, uc
            )
