...
class MixDegradationDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.opt_codeformer = opt['codeformer_config']
    ...
    def __getitem__(self, index):
        ...
        elif degradation_type == 'codeformer':
            return self.__getitem_codeformer__(index)
        else:
    ...
    def __getitem_realesrgan__(self, index):
        ...
        return dict(jpg=hq, txt="", hint=lq, gt_path=gt_path )
    ...
    def __getitem_codeformer__(self, index):
        ...
        while retry > 0:
            try:
                if self.opt_codeformer['use_crop']:
                    pil_img_gt = utils.center_crop_arr(Image.open(gt_path).convert("RGB"), self.opt_codeformer['out_size'])
                else:
                    pil_img_gt = np.array(Image.open(gt_path).convert("RGB"))
                    assert pil_img_gt.shape[:2] == (self.opt_codeformer['out_size'], self.opt_codeformer['out_size'])
            except Exception as e:
                logger = get_root_logger()
                logger.warning(f'dataloader error: {e}, remaining retry times: {retry - 1}')
                # change another file to read
        ...
        img_gt, status = augment(img_gt, hflip=self.opt_codeformer['use_hflip'], rotation=False, return_status=True)
        h, w, _ = img_gt.shape

        # ------------------------ generate lq image ------------------------ #
        # blur
        kernel = degradations.random_mixed_kernels(
            self.opt_codeformer['kernel_list'],
            self.opt_codeformer['kernel_prob'],
            self.opt_codeformer['blur_kernel_size'],
            self.opt_codeformer['blur_sigma'],
            self.opt_codeformer['blur_sigma'], [-math.pi, math.pi],
            noise_range=None)
        img_lq = cv2.filter2D(img_gt, -1, kernel)
        # downsample
        scale = np.random.uniform(self.opt_codeformer['downsample_range'][0], self.opt_codeformer['downsample_range'][1])
        img_lq = cv2.resize(img_lq, (int(w // scale), int(h // scale)), interpolation=cv2.INTER_LINEAR)
        # noise
        if self.opt_codeformer['noise_range'] is not None:
            img_lq = degradations.random_add_gaussian_noise(img_lq, self.opt_codeformer['noise_range'])
        # jpeg compression
        if self.opt_codeformer['jpeg_range'] is not None:
            img_lq = degradations.random_add_jpg_compression(img_lq, self.opt_codeformer['jpeg_range'])
