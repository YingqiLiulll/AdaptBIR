...
def degradation_bsrgan_plus(img, sf=4, shuffle_prob=0.5, use_sharp=True, isp_model=None):
    ...
    img = cv2.resize(img, hq.shape[:2][::-1], interpolation=random.choice([1, 2, 3]))
...
class BSRGANPlusDataset(data.Dataset):
    
    def __init__(self, opt):
        super(BSRGANPlusDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        sf = np.random.uniform(self.downsample_range[0], self.downsample_range[1])
        # hwc, rgb, 0,1
        img_lq = degradation_bsrgan_plus(img_gt, sf, use_sharp=False, isp_model=None)
        
        # BGR to RGB, [-1, 1]
        target = ((img_gt - 0.5) / 0.5).astype(np.float32)
        # BGR to RGB, [0, 1]
        source = img_lq.astype(np.float32)
