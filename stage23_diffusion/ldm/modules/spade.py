...
class SPADE(nn.Module):
    ...
    def forward(self, x, segmap, size=None):
...
class STABLE_SPADE(nn.Module):
    ...
    def forward(self, x_dic, segmap_dic, size=None):
        ...
        if size is None:
            segmap = segmap_dic[str(x_dic.size(-1))]
            # print("str(x_dic.size(-1))",str(x_dic.size(-1))) # 64
            # print("segmap_size",segmap.size()) #get s_cond's key 64, size [6,256,64,64]
            x = x_dic
        else:
            x = x_dic[str(size)]
            segmap = segmap_dic[str(size)]
